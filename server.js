...
// Trust Vercel's proxy headers (X-Forwarded-Proto, X-Forwarded-Host) so
// req.protocol correctly reports "https" instead of the default "http" —
// otherwise personal MCP links generated by /auth come out as http://,
// which clients like ChatGPT reject outright as an unsafe URL.
...
app.post("/auth", async (req, res) => {
  ...
  try {
    ...
    // Protocol: trust the (now-trusted) req.protocol, but fall back to https unless
    // we're clearly on localhost — belt-and-braces against ever generating an
    // http:// link again, which clients like ChatGPT reject as unsafe.
    const isLocalhost = /^(localhost|127\.0\.0\.1)/.test(req.get("host") || "");
